...
async function initDB() {
  ...
  try {
    const adminEmail = 'nashwann1991@gmail.com';
    const existingAdmin = db.prepare('SELECT * FROM users WHERE email = ?').get(adminEmail);
    
    if (!existingAdmin) {
      const hashedPassword = await bcrypt.hash('19912001', 10);
      const uid = 'admin-nashwan';
      const createdAt = Date.now();
      db.prepare('INSERT INTO users (uid, email, password, displayName, role, createdAt) VALUES (?, ?, ?, ?, ?, ?)').run(uid, adminEmail, hashedPassword, 'Nashwan Admin', 'admin', createdAt);
...
app.post('/api/leaves', async (req, res) => {
  ...
  try {
    ...
    const id = 'GSL-' + Math.random().toString(36).substr(2, 6).toUpperCase();
    const createdAt = Date.now();
