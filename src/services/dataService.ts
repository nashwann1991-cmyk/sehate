...
export const dataService = {
  ...
  addLeave: async (leave: Omit<SickLeave, 'id' | 'createdAt' | 'status' | 'qrCodeData' | 'leaveNumber'> & { leaveNumber?: string }): Promise<SickLeave> => {
    const path = 'sick_leaves';
    try {
      const now = new Date();
      const dateStr = now.toISOString().split('T')[0].replace(/-/g, '');
      const randomStr = Math.random().toString(36).substr(2, 4).toUpperCase();
      const generatedLeaveNumber = `GSL-${dateStr}-${randomStr}`;
      
      const leaveNumber = leave.leaveNumber || generatedLeaveNumber;
