...
export default function DoctorDashboard({ user }: DoctorDashboardProps) {
  ...
  const generateLeaveNumber = () => {
    ...
    return `GSL-${dateStr}-${randomStr}`;
  };

  const [currentLeaveNumber, setCurrentLeaveNumber] = useState(generateLeaveNumber());
  ...
  const handleIssueLeave = async (e: FormEvent) => {
    ...
    try {
      ...
      const newLeave = await dataService.addLeave({
        leaveNumber: currentLeaveNumber,
      ...
      setCurrentLeaveNumber(generateLeaveNumber());
      setFormData({
        patientName: '',
        patientNameEn: '',
        patientId: '',
            ...
            {!successLeaveId && (
              <form onSubmit={handleIssueLeave} className="space-y-8">
                ...
                <div className="space-y-6">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 border-b border-slate-100 pb-2 mb-4">
                    ...
                    <div className="bg-slate-100 px-3 py-1 rounded-lg border border-slate-200">
                      <span className="text-xs font-bold text-slate-500 ml-2">رقم الإجازة:</span>
                      <span className="text-sm font-mono font-bold text-medical-primary">{currentLeaveNumber}</span>
                    </div>
                  </div>
